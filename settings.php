 ...
 * 
 * $Id: settings.php,v 1.2 2006/06/04 17:25:57 garrett Exp $
 * 
 ******************************/
...
if ( isset($_POST['submit']) )
{
    ...
    if ( !empty($_POST['member_ctprofile']) )
    {
	    $fv->is_number(array(
    	    'member_ctprofile' => $user->lang['fv_number']));
    }
...
switch ( $action )
{
    ...
    case 'update':
		...
		if (!empty($_POST['member_ctprofile'])) {
			...
			$sql = "UPDATE ".MEMBERS_TABLE."
			           SET member_ctprofile = '".$_POST['member_ctprofile']."'
					 WHERE member_id = '".$member_id."'";
			
			if ( !($result = $db->query($sql)) )
			{
				message_die('Could not update member information', '', __FILE__, __LINE__, $sql);
			}
		}
    ...
    case 'account_settings':
		...
		// get the CTProfile if it exists
		$sql = "SELECT member_id
				  FROM ".MEMBER_USER_TABLE." mu,
				       ".USERS_TABLE." u
				 WHERE mu.user_id = u.user_id
				   AND u.username = '".$user->data['username']."'";
		$member_id = $db->query_first($sql);

		if (!empty($member_id)) {
			$sql = "SELECT member_ctprofile
					  FROM ".MEMBERS_TABLE."
					 WHERE member_id = '".$member_id."'";
			$member_ctprofile = $db->query_first($sql);
		}
        ...
        $tpl->assign_vars(array(
            ...
            'S_LINKED_MEMBER' => ( !empty($member_id) ) ? true : false,
			...
			'L_CTPROFILE' => $user->lang['ctprofile'],
			'L_CTPROFILE_NOTE' => $user->lang['ctprofile_note'],
            ...
            'MEMBER_CTPROFILE' => $member_ctprofile,
            ...
            'FV_CTPROFILE' => $fv->generate_error('member_ctprofile'),
