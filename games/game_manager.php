 ...
 * Date:        $Date: 2008-05-17 16:52:00 -0700 (Sat, 17 May 2008) $
 * -----------------------------------------------------------------------
...
//gehAVATARS
    /**
     * Return the icon name for each avatar icon. We use the same naming convention as blizzard to hopefully
     * ease the update of the icons as they evolve through each expansion
     *
     * @param     string     $race         The string race name
     * @param     string     $class        The string class name
     * @param     string     $gender       The string gender name
     * @param     int        $level        The integer level
     */
    function get_avatar_icons($race='unknown', $class='unknown', $gender='male', $level=1)
    {
		/**
        ...
        $gamedata = $this->get_game_data('wow');
		
		// Class & Race are stored with underscore separators so we need to add the underscore for EQdkp compatibility
		$class = str_replace(' ','_',$class);
		$race = str_replace(' ','_',$race);
		$avatar_icons = array('race' 	 => $gamedata['data']['races'][strtolower($race)]['blizz_id'].'-'.$gamedata['data']['genders'][strtolower($gender)]['blizz_id'],
		    		          'class' 	 => $gamedata['data']['classes'][strtolower($class)]['blizz_id'],
					  	      'portrait' => $gamedata['data']['genders'][strtolower($gender)]['blizz_id'].'-'.$gamedata['data']['races'][strtolower($race)]['blizz_id'].'-'.$gamedata['data']['classes'][strtolower($class)]['blizz_id'],
					          'icon_dir' => $icon_dir,
							  );
    ...
    /**
     * Return the icon name for a class. We use the same naming convention as blizzard to hopefully
     * ease the update of the icons as they evolve through each expansion
     *
     * @param     string     $class        The string class name
     */
	function get_class_icon ($class) {
		...
		$avatar_icons = $this->get_avatar_icons('unknown',$class);
		
		return $avatar_icons['class'];
	}

    /**
     * Return the CSS style for a class. 
     *
     * @param     string     $class        The string class name
     */
	function get_class_style ($class = 'Unknown') {

        $gamedata = $this->get_game_data('wow');
		
		// Class & Race are stored with underscore separators so we need to add the underscore for EQdkp compatibility
		$class = str_replace(' ','_',$class);
		$class_style = array('color'	=> $gamedata['data']['classes'][strtolower($class)]['color'],
		                     'css' 		=> str_replace(" ","-",strtolower($gamedata['data']['classes'][strtolower($class)]['name'])),
						    );
    ...
    /**
     * Return the icon name for a race. We use the same naming convention as blizzard to hopefully
     * ease the update of the icons as they evolve through each expansion
     *
     * @param     string     $race         The string race name
     * @param     string     $gender       The string gender name
     */
	function get_race_icon ($race,$gender) {

		$avatar_icons = $this->get_avatar_icons($race,'unknown',$gender);
		
		return $avatar_icons['race'];
	}
//gehEND
}
?>