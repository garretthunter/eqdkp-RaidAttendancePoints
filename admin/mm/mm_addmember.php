...
//gehEND
        ...
        if ( !empty($this->url_id) )
        {
            ...
            $this->member = array(
//gehALTERNATES
                'member_gender'     => $in->get('member_gender',$row['member_gender']),
//gehEND
...
//gehALTERNATES
            ...
            '{L_GENDER_AFTER}'      => $this->find_difference($this->old_member['member_gender'],     $in->get('member_gender')),
//gehALTERNATES
...
//gehALTERNATES
    ...
    function process_delete_alternates()
    {
        ...
        if ($in->exists(URI_ALTERNATE_ID)) {
            ...
            // Success message
            //
            $success_message = sprintf($user->lang['admin_delete_alternates_success'], $alternate_name_list, $in->get('member_name'));
        }
        $link_list = array(
            $user->lang['list_edit_del_member'] => path_default('admin/manage_members.php') . path_params('mode', 'list')
            );
...
// gehALTERNATES
        ...
        foreach ($genders as $gender) {
            $tpl->assign_block_vars('gender_row', array(
                'VALUE'    => $gender,
                'SELECTED' => ( option_selected($this->member['member_gender'] == $gender)),
                'OPTION'   => $gender
            ));
        }
...
// gehALTERNATES
        ...
        foreach ($alt_names as $key => $name) {
            ...
            $tpl->assign_block_vars('alternates_row', array(
                ...
                'RACE'      => $name['race'],
                'CLASS'     => $name['class'],
                'U_VIEW_MEMBER' => edit_member_path($name['name'])
            ));
        }
...
//gehALTERNATES
            'F_DELETE_ALTERNATES'   => path_default('admin/manage_members.php') . path_params('mode', 'deletealternates'),
