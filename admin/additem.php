...
//geh
        ...
        $query = $db->build_query('INSERT', array(
            ...
            'game_item_icon'    => $this->item['item_icon'])
        );
        $db->query("INSERT INTO __game_items {$query}");
//gehEND
    }
    ...
    function getWoWHeadItem ($itemIDorName) {
		...
		if ($error->length == 0 ) { 
			...
			foreach ($items as $item) {
				$wowheadItem['item_game_id'] = $item->getAttribute('id');
			}
			
			$qualitys = $doc->getElementsByTagName('quality');
			foreach ($qualitys as $quality) {
				$wowheadItem['item_quality'] = $quality->getAttribute('id');
			}
...
// Items found at http://static.wowhead.com/images/icons/large/inv_hammer_07.jpg
            ...
            $wowheadItem['error'] = $error->item(0)->nodeValue;
