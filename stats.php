 ...
 *
 * $Id: stats.php,v 1.3 2006/07/09 19:32:28 garrett Exp $
 *
 ******************************/
...
// --------------------------------------------------
// Set Item Stats -----------------------------------
include_once($eqdkp_root_path.'plugins/setprogress/config.php');

// 1) Get the member's class
	$sql = "SELECT class_name
	          FROM ". CLASS_TABLE . " c,
			       ". MEMBERS_TABLE ." m
		     WHERE class_id = member_class_id
			   AND m.member_name = '".$row['member_name']."'";
	$class_name = $db->query_first($sql);

// 2) Based on class, build the complete list of Set Items a member could own
	for ($i = 1; $i <=3; $i++) {
		$sql = "SELECT COUNT(*)
				  FROM ".ITEMS_TABLE."
		...
		$items = "";
		foreach ($setitems[$class_name]["tier${i}"]["set"] as $key => $item) {
			$items .= "'".mysql_real_escape_string($item)."',";
		}
		$sql .= substr($items,0,strlen($items)-1).")";

		$count = $db->query_first($sql);
		$armorSetCount[$i]["owned"] = $count;
		$armorSetCount[$i]["total"] = count($setitems[$class_name]["tier${i}"]["set"]);
	}
// Set Item Stats -----------------------------------
// --------------------------------------------------
...
/* Set Item Stats */	
		'TIER_1' => $armorSetCount[1]["owned"]." / ". $armorSetCount[1]["total"],
		'TIER_2' => $armorSetCount[2]["owned"]." / ". $armorSetCount[2]["total"],
		'TIER_3' => $armorSetCount[3]["owned"]." / ". $armorSetCount[3]["total"],
/* Set Item Stats */	
